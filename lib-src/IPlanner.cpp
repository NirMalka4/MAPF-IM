...
{
    return std::string(log_observation_prefix) + std::string(log_lines_delimiter) + (is_maybe_open ? std::string(log_observation_maybe_opened) : std::string(log_observation_maybe_blocked)) + 
...
void IPlanner::LogSnapshot(const Snapshot& snap)
{
    ...
    LogMaybeEdges(snap.GetMaybeBlockedEdges(), log_Maybe_blocked_title);
    LogMaybeEdges(snap.GetMaybeOpenEdges(), log_Maybe_Opened_title);
    log << log_lines_delimiter;
}

void IPlanner::LogMaybeEdges(const EdgeSet& maybe_edges, const std::string& title)
{
    log << title << log_lines_delimiter;
